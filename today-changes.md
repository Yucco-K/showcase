# 2025 年 7 月 27 日の変更内容

## チャットボット API のリアーキテクチャ (Python + LangChain)

- **概要**: 従来の Deno ベースのチャットボット API を、メンテナンス性と拡張性を目的として Python スタックに刷新。LangChain を導入し、RAG（Retrieval-Augmented Generation）ベースの高度な応答システムを構築しました。(Closes #20)
...
- **開発サーバー**: ローカル開発環境として`uvicorn`を導入。

### 2. RAG (Retrieval-Augmented Generation) の実装と改善

- **ベクトル検索**: `Supabase` (`pgvector`) に保存された商品情報やドキュメントのベクトル埋め込みを利用し、ユーザーの質問に関連する情報を高速に検索。
- **ナレッジベース拡張**: `update-knowledge-base.ts`スクリプトを改良し、複数の Markdown ドキュメント（FAQ、利用規約など）を知識源として一括で取り込めるように拡張。
- **実装の変遷**: 当初`RetrievalQA`チェーンを利用していましたが、安定性と制御性を向上させるため、より直接的な RAG パイプラインにリファクタリング。

### 3. API と開発環境の安定化

- **API ルーティング**: `FastAPI`のルーターを導入し、Vercel の`rewrites`設定と連携させることで、フロントエンドの SPA ルーティングとの競合を解消。`CORS`エラーもミドルウェアで対応済み。
- **環境変数管理**: Vercel 環境での環境変数読み込み問題を解決するため、非同期のシングルトンパターンによる設定初期化処理を実装。
- **エラーハンドリングとロギング**: `supabase-py v2`の仕様に合わせた RPC 呼び出しのエラーハンドリングを強化。また、デバッグ用の`print`文を標準の`logging`モジュールに統一し、構造化されたログ出力を実現。

### 4. フロントエンド連携

- **API エンドポイント変更**: フロントエンドのチャットコンポーネントが、新しくデプロイされた Python ベースの API エンドポイントを参照するように修正。
- **React 警告修正**: `styled-components`に起因する React の警告を解消し、開発者コンソールをクリーンアップ。
...
- **概要**: 統計データの信頼性向上と UI/UX の改善を目的として、マーケティングダッシュボードの大規模なリファクタリングを実施しました。(Closes #10)

### 1. データ集計ロジックの刷新 (Supabase への移行)

- **変更点**: これまで`localStorage`に依存していた総購入数や総収益の集計ロジックを、`Supabase`から直接全購入履歴を取得する方法に変更。
- **実装**: `usePurchaseHistory`フックに非同期関数`getAllPurchaseHistory`を新設し、本番環境でも正確なデータが表示されるように修正。

### 2. Gorse 推薦エンジンとの実データ連携

- **変更点**: ダミーデータや部分的なデータではなく、`Gorse` API から直接フィードバック統計（いいね、購入など）を取得するように統合。
- **実装**: Gorse の`/api/users`および`/api/items`エンドポイントを正しく呼び出すように修正し、リアルタイムの統計データをダッシュボードに表示。

### 3. UI/UX の高度化

- **スクロール位置復元機能**: `useScrollRestoreOnStateChange`というカスタムフックを新規に実装。タブ切り替えやページネーション後もスクロール位置が保持され、ユーザー体験が向上。このフックはモバイルのタッチイベントにも対応済みです。
- **ローディング UI の改善**: 従来のスピナー表示を廃止し、`Skeleton UI`（スケルトンローダー）を導入。データの読み込み状態が視覚的に分かりやすくなりました。
- **時系列チャートのインタラクティブ化**: 日/週/月での表示粒度切り替え機能と、ページネーション機能を追加。

### 4. コード品質とモバイル対応の強化

- **型安全性**: TypeScript の型定義を強化し、データ集計関連の関数の安全性を向上。
- **Linter/未使用コード**: linter エラーを解消し、不要なコンポーネントを削除。
- **モバイル対応**: スクロール復元機能は iOS/Android での動作を考慮した遅延調整が実装されており、タッチデバイスでの操作性が向上。
